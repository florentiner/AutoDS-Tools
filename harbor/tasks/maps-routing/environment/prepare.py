"""Stage maps-routing (TabReD) data — synthetic, temporal 80/20 split (build time).
...
SEED = int(hashlib.sha256(b"maps-routing").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] maps-routing kind={KIND} train={len(train)} test={len(holdout)}")
