"""Stage homesite-insurance (TabReD) data — synthetic, temporal 80/20 split (build time).
...
ID, TARGET, KIND = "id", "target", "binary"
# Seed from the task name so every task gets a DISTINCT dataset + signal.
SEED = int(hashlib.sha256(b"homesite-insurance").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] homesite-insurance kind={KIND} train={len(train)} test={len(holdout)}")
