"""Stage ecom-offers (TabReD) data — synthetic, temporal 80/20 split (build time).
...
SEED = int(hashlib.sha256(b"ecom-offers").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] ecom-offers kind={KIND} train={len(train)} test={len(holdout)}")
