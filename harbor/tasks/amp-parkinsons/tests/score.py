"""Score amp-parkinsons (R2). Always writes a reward."""
...
def main() -> int:
    ...
    if ID not in sub.columns or TARGET not in sub.columns:
        return emit(0.0, error="submission needs columns [id, updrs]")
    m = ans.merge(sub, on=ID, suffixes=("_true","_pred"))
    if m.empty: return emit(0.0, error="no id overlap")
    yt = pd.to_numeric(m[f"{TARGET}_true"],errors="coerce").to_numpy(); yp = pd.to_numeric(m[f"{TARGET}_pred"],errors="coerce").to_numpy()
    ...
    return emit(max(0.0,r2), r2=r2, rmse=float(np.sqrt(np.mean((yt-yp)**2))), n=int(len(yt)))
