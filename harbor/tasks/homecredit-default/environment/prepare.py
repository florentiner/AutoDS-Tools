"""Stage homecredit-default (TabReD) data — synthetic, temporal 80/20 split (build time).
...
SEED = int(hashlib.sha256(b"homecredit-default").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] homecredit-default kind={KIND} train={len(train)} test={len(holdout)}")
