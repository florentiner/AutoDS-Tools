"""Stage sberbank-housing (TabReD) data — synthetic, temporal 80/20 split (build time).
...
SEED = int(hashlib.sha256(b"sberbank-housing").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] sberbank-housing kind={KIND} train={len(train)} test={len(holdout)}")
