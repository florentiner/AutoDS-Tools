"""Stage cooking-time (TabReD) data — synthetic, temporal 80/20 split (build time).
...
SEED = int(hashlib.sha256(b"cooking-time").hexdigest(), 16) % (2**32)
...
def main() -> None:
    ...
    print(f"[prepare] cooking-time kind={KIND} train={len(train)} test={len(holdout)}")
